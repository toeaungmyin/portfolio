				...
				<div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-12 lg:gap-10">
					<div className="lg:col-span-5">
						<Link href="/" className="group mb-6 inline-flex items-center gap-3" aria-label="Go to home">
							...
							<span className="text-sm font-bold uppercase tracking-[0.2em] text-white">{displayName}</span>
						</Link>
						<p className="max-w-sm text-sm font-light leading-relaxed text-[#555555]">
							<span className="font-medium text-[#6a6a6a]">{profile.role}</span>
					...
					<div className="lg:col-span-3">
						<p className="mb-4 text-[10px] font-black uppercase tracking-[0.35em] text-ink-eyebrow">Navigate</p>
						<nav className="flex flex-col gap-2" aria-label="Footer">
							{navLinks.map((item) => (
								<Link
									key={item.href}
									href={item.href}
									className="w-fit text-[11px] font-bold uppercase tracking-[0.2em] text-[#888888] transition-colors hover:text-white"
								>
									{item.label}
								</Link>
							))}
					...
					<div className="sm:col-span-2 lg:col-span-4">
						<p className="mb-4 text-[10px] font-black uppercase tracking-[0.35em] text-ink-eyebrow">Connect</p>
						<div className="flex flex-wrap gap-2">
							{socialLinks.map(({ label, href, Icon }) => (
								<a
									...
									className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/3 text-[#888888] transition-all hover:border-white/20 hover:bg-white/5 hover:text-white"
									aria-label={label}
								>
